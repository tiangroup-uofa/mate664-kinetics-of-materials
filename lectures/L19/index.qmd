...
The repulsion between two spherical particles separated by $D$ under the Derjarguin approximation is

$$
V_{\mathrm{R}}^{\mathrm{ss}}(D)
= 64 \pi k_B T c_{0} R \kappa^{-2} \Lambda_{0}^{2} e^{-\kappa D}
\end{equation}
$$

Such approximation is valid when:

- $R \gg D$, i.e. near the junction the surfaces from two particles are still considered as planar.
- $D \gg \kappa^{-1}$, i.e. small overlap between EDLs.
