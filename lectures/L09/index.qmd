...
```{=tex}
\begin{align}
D &= \sqrt{\dfrac{k_B T}{2 \pi m}} \frac{(L_w + L_a)^2}{L_w} e^{-\dfrac{E^a}{k_B T}} \\
  &= D_0 e^{-\dfrac{E^a}{k_B T}}
\end{align}
```

## More Complex Landscape: Parabolic Well Model

- Near the bottom of well,
...
```{=tex}
\begin{align}
\Gamma' &= \frac{1}{2 \pi} \sqrt{\frac{\beta}{m}}
e^{-\frac{E^a}{k_B T}}
\end{align}
```
...
- General solution still follows the Boltzmann-distribution-like equation (see _KOM eq. 7.25_ )

```{=tex}
\begin{align}
\Gamma' &= \nu e^{-\frac{G^m}{k_B T}} \\
        &= \nu e^{\frac{S^m}{k_B}} e^{-\frac{H^m}{k_B T}} \\
\end{align}
```
